...
type AssetInput struct {
	...
	// Components, when set, replaces the asset's component list atomically on
	// save. If left nil, the existing component rows are kept (and a default
	// single-piece row is auto-created on create when none exist yet).
	Components       []AssetComponentInput `json:"components,omitempty"`
	HasComponentList bool                  `json:"-"`
}

// AssetComponentInput is the wire-level shape for one component supplied in
// an AssetInput payload (multipart `components` JSON string or REST body).
